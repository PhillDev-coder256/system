<?php
    ...
    if($_GET){
        ...
        $id=$_GET["id"];
        $result001= $database->query("select * from wishlist where id=$id;");
        ...
        $backupsql = "INSERT INTO deleted_wishlist(old_id, date_inserted, amount, comment, email)VALUES('$id', '$date_inserted', '$amount', '$comment', '$email')";

        $excute_backup = mysqli_query($database, $backupsql);

        
        if($excute_backup){
            $sql= "delete from wishlist where useremail='$email' and id = $id;";
            ...
            echo "Couldn't backup wishlist record, therefore can't delete !!";
        }
